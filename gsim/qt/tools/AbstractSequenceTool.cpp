...
#include <gsim/qt/item/OperationsView.hpp>
...
AbstractSequenceItem::AbstractSequenceItem(
        ...
        QWidget * parent) : 
    qt::AbstractItemFrame(parent), 
    m_connection(connection), 
    m_message(message)
{
    const QString& id = connection->name();
    ...
    QLabel * title = new QLabel(
        QString("<b>Connection: '%1' Message: '%2'</b>")
            .arg(id).arg(getMessageName(message))); 
...
Connection_ptr AbstractSequenceItem::connection() const
{
    return m_connection;
}

TypeDescriptor_ptr AbstractSequenceItem::message() const
{
    return m_message;
}
...
void AbstractSequence::removeInstance(Connection_ptr con)
{
...
AbstractSequenceTool::AbstractSequenceTool(QWidget * parent) :
    AbstractTool(parent), 
    m_model(MessageModel::kOut, this),
    m_moveToSignalMapper(NULL), 
    m_currentItem(NULL)
{
    ...
    // Signals
    connect(m_view, 
            SIGNAL(selectedOperation(Connection_ptr,
                    TypeDescriptor_ptr)),
            this,
            SLOT(appendAbstractItem(Connection_ptr, 
                    TypeDescriptor_ptr)));
...
void AbstractSequenceTool::registerInstance(Connection_ptr connection)
{
    m_model.registerInstance(connection);
}

void AbstractSequenceTool::unregisterInstance(Connection_ptr id)
{
		...
		TypeDescriptor_ptr op)
{
    ...
    AbstractSequenceItem * item = 
        createAbstractItem(connection, op);
...
void AbstractSequenceTool::loadSequence()
{
    ...
    {
        QMessageBox::critical(this, "Error", "Invalid sequence file.");
    }
}
...
void AbstractSequenceTool::duplicateCurrentItem()
{
    ...
    if (m_currentItem && idx > -1 && idx < m_sequences.size())
    {
        ...
        AbstractSequenceItem * item = 
            createAbstractItem(m_currentItem->connection(), 
...
void AbstractSequenceItem::save(QVariant& settings)
{
    ...
    map["connection"] = m_connection->name();
    map["message"] = getMessageName(m_message);
...
void AbstractSequenceTool::load(const QVariant& settings)
{
            ...
            it != list.end(); it++, i++) 
    {
                ...
                lit != seqList.end(); lit++) 
        {
            ...
            const QString obj = map["connection"].toString();
            const std::string message = 
                map["message"].toString().toStdString();

            Connection_ptr connection = m_model.getInstance(obj);
            TypeDescriptor_ptr op = NULL;

            if (connection && (op = 
                     connection->descriptor()->out()->get_descriptor_by_name(message.c_str())))
            {
                appendAbstractItem(connection, op)->load(*lit);
            }
        }

        seq->load(*it);

        // Actualiza el nombre de la secuencia en la pestaña
