...
ParametersFromFilesTool::ParametersFromFilesTool(QWidget * parent) :
    QWidget(parent), m_model(this), m_message(NULL)
{
...
{
    m_model.initialize(messageName, reflective);

    m_connection = connection;
    m_message = reflective;
        ...
        const QList< int >& path)
{
    ...
    if (m_message && m_connection)
    {
        ...
        plot = new FilesItem(m_connection, m_message, path);
        ...
        QString title(m_messageName);
        title += getFieldName(m_message, path);
        item->setTitle(title);
...
void ParametersFromFilesTool::deleteRequested(qt::SortableGroupItem* item)
{
    ...
    if (plot)
    {
        m_model.uncheck(plot->getReflective(),
                plot->path());
...
void ParametersFromFilesTool::clear()
{
    ...
    {
        // notify to model
        m_model.uncheck(m_message, (*it)->path());
    }
...
TypeDescriptor_ptr FilesItem::getReflective() const
{
    return m_reflective;
}
...
void FilesItem::save(QVariant& settings)
{
    ...
    map["message"] = getMessageName(m_reflective);
